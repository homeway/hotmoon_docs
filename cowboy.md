Cowboy框架
==========
* [按照官方指南创建cowboy项目](#按照官方指南创建cowboy项目)
* [使用shell和rebar管理cowboy项目](#使用shell和rebar管理cowboy项目)
...
###添加应用依赖cowboy到`src/hello_erlang.app.src`
这会要求应用启动前必须先启动cowboy，添加后的app配置如下（顺便修改了应用描述和版本信息）：
...
在emacs中，首先执行`C-c C-z`，启动`erlang shell`，然后执行：
